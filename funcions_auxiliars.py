    ...
    if model_id == "gemini-2.5-flash-preview-05-20":
        ...
        generate_content_config = types.GenerateContentConfig(
            ...
            thinking_config=types.ThinkingConfig(
                # thinking_budget=round(max_output_tokens * 0.3),
                thinking_budget=0,
            ),
        )
        ...
        # El base_url i la versió de l'API poden variar segons els teus models afinats
        ...
        response.raise_for_status()  # Llença una excepció per errors HTTP 4xx/5xx
...
def benchmark_model(model_id, model_name, num_questions=10, temperature=0.6, k_shot=5):
    ...
    # Example explanations hardcoded (mateix que l'original)
    ...
    correct = 0
    no_answer_count = 0  # Renamed from no_answer to avoid conflict
    responses_data = []  # Renamed from responses

    for idx, ex in tqdm(enumerate(test_examples), total=len(test_examples), desc=f"Benchmarking {model_name}"):
        prompt = (
        ...
        if model_ans is None:  # Si no hi ha resposta, intenta amb més tokens
            ...
            if model_ans is None:
                ...
                continue  # Salta a la següent pregunta
    ...
    valid_responses = len(test_examples) - no_answer_count
    accuracy = (correct / valid_responses * 100) if valid_responses > 0 else 0
    print(f"No answer provided by model {model_name}: {no_answer_count} out of {len(test_examples)}")
    print(f"Accuracy for {model_name}: {correct}/{valid_responses} = {accuracy:.1f}%")

    return {
        "model_name": model_name,
        "accuracy": accuracy,
        "correct_count": correct,  # Renamed from correct
        "no_answer_count": no_answer_count,
        "total_questions": len(test_examples),  # Renamed from total
        "responses_data": responses_data,  # Renamed from responses
    }
